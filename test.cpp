...
void test(){
	...
	string str2 = "123 45";
	cout << str2.size() << endl;
	// 下卖弄这两个返回一样 并且是超大的值 表示字符串最长是多长
	cout << str.max_size() << endl;
	cout << str2.max_size() << endl;

	str2.clear(); // 清空当前字符串 大小置为0
	cout << str2.size() << endl; 
	cout << str2.empty() << endl; // 是否为空
}
void test2(){
	// capacity： 当前对象可以存放的最大字符数
	// 容量(capacity) >= 大小（size)
	string str = "12345";
	int capacity = str.capacity(); // 容量 15
	int size = str.size(); // 大小
	str.push_back('a'); // 给当前字符串后面加一个字符 +1
	for (int i = 0; i < 10; ++i)
		str.push_back('b');  // +10
	capacity = str.capacity(); // 15*2+1 = 31

	// resize: 修改有效字符的个数：
	// 1. 减小有效字符的个数， 多余的删掉
	// 2. 减小有效字符个数, 如果没有新的空闲的位置， 则第二个参数无用
	// 3. 增加有效字符的个数， 增加的位置为第二个参数的缺省内容
	// 4. 增加有效字符的个数， 新增的位置赋值为第二个参数内容
	str.resize(10); // 减小当前字符串大小为当前值 变成12345abbbb
	size = str.size();
	capacity = str.capacity(); // 容量不变

	str.resize(30, 'c'); // 给后面添加‘c’，到大小为30
	size = str.size(); // 变30， 后面
	capacity = str.capacity(); // 容量不变

	str.resize(15, 'd'); // 12345abbbbccccc到15个，后面的d没作用
	size = str.size(); // 15
	capacity = str.capacity(); // 不变

	// resize: 默认赋值为‘\0’
	str.resize(30); // 给后面添加了缺省的‘\0’，到大小为30 12345abbbbccccc00000000...
	size = str.size(); // 变30 
	capacity = str.capacity(); // 容量不变
	...
	str.resize(60);
	size = str.size();
	capacity = str.capacity(); // 63
}
void test3(){
	string s1 = "123";
	int capacity = s1.capacity();
	//reserse(储备): 增加容量, 只增加容量,  reverse(颠倒)
